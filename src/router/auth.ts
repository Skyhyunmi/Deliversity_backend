...
auth.post('/signup', function (req:any, res:Response, next:NextFunction) {
  req.query=null;
  passport.authenticate('signup', function (err: any, user: any, info: { message: any; }) {
    if (err) {
...
auth.post('/login', function (req:any, res:Response, next:NextFunction) {
  req.query = null;
  passport.authenticate('login', { session: false }, function (err: any, user: any, info: { message: string; }) {
    if (info) return res.status(403).json(util.successFalse(null, info.message,null));
    if (err || !user) {
    ...
    req.logIn(user, { session: false }, function (err:any) {
      ...
      user.authToken = jwt.sign(payload, process.env.JWT_SECRET as jwt.Secret, { expiresIn: 60 * 90 });
...
auth.get('/refresh', util.isLoggedin, function (req:any, res:Response) {
  db.User.findOne({ where: { user_id: req.decoded.id } }).then(function (user:any) {
    if (!user) {
